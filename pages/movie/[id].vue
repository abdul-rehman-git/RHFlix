        ...
        <div class="flex flex-col md:flex-row gap-6 sm:gap-8 items-start">
          ...
          <div class="flex-1 space-y-4 sm:space-y-5 text-center md:text-left">
            ...
            <div class="flex flex-wrap items-center justify-center md:justify-start gap-2 sm:gap-2.5 text-xs font-semibold">
              ...
              <div v-if="movie.vote_average && movie.vote_average > 0" class="flex items-center space-x-1 px-2.5 py-1 bg-black/60 backdrop-blur-md rounded-lg text-marxi-gold border border-white/10">
              ...
              <span v-if="movie.release_date" class="px-2.5 py-1 bg-marxi-800 rounded-lg text-gray-300 border border-marxi-700">
                {{ isUnreleased ? `Expected: ${movie.release_date}` : movie.release_date }}
              </span>
            ...
            <div v-if="isUnreleased" class="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-amber-500/15 via-amber-900/10 to-transparent border border-amber-500/30 text-amber-200 text-xs sm:text-sm flex items-start space-x-3 shadow-lg">
              ...
              <div class="space-y-1 text-left">
                <p class="font-bold text-white text-xs sm:text-sm">
                  This title is not yet available for streaming.
                </p>
                <p class="text-gray-300 text-xs leading-relaxed">
                  Scheduled release: <strong class="text-amber-400">{{ formattedReleaseDate(movie) }}</strong>. Streaming servers do not have this title yet. You can watch the official trailer below or bookmark it to your list!
                </p>
              </div>
            </div>
            ...
            <div class="pt-2 flex flex-wrap items-center justify-center md:justify-start gap-3 sm:gap-4">
              ...
              <!-- Unreleased: Primary Trailer / Coming Soon Button (NO Watch Movie Link!) -->
              <button 
              ...
              <!-- Secondary Trailer Button (for released movies that have trailer) -->
              <button 
                v-if="!isUnreleased && trailerKey"
                @click="isTrailerOpen = true"
