<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-8 sm:space-y-10">
  <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-6">
    
    <!-- Hero Header -->
    <div class="space-y-3 text-center sm:text-left border-b border-marxi-800 pb-6">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div class="inline-flex items-center space-x-2 px-3 py-1 bg-marxi-accent/10 border border-marxi-accent/30 rounded-full text-marxi-accent text-xs font-bold uppercase tracking-wider mb-2">
            <span class="w-2 h-2 rounded-full bg-marxi-accent animate-pulse"></span>
          ...
          <p class="text-xs sm:text-sm text-gray-400 mt-1 max-w-2xl">
            Stream K-Dramas, Anime, Action, Romance, Bollywood, Turkish Serials, 18+ Mature content, and top genres on RHFlix.
          </p>
        </div>
    ...
    <div class="flex items-center justify-between gap-3 border-b border-marxi-800/80 pb-3">
      <div>
        ...
        <button 
          ...
          @click="resetFilters" 
          class="inline-flex items-center space-x-1.5 px-4 py-2 bg-marxi-850 hover:bg-marxi-800 text-marxi-gold text-xs font-bold rounded-xl border border-marxi-gold/30 transition-all shrink-0 self-center sm:self-auto min-h-[40px]"
        >
    ...
    <!-- Quick Presets Row (Featured Collections) -->
    <div class="space-y-3">
      <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center space-x-2">
    ...
    <div class="sticky top-16 z-30 space-y-3 bg-marxi-950/90 backdrop-blur-xl p-3 sm:p-4 rounded-2xl border border-white/10 shadow-2xl">
      ...
      <div class="flex items-center space-x-2 overflow-x-auto hide-scrollbar pb-1">
        <button 
          ...
          class="group relative p-3 rounded-2xl border transition-all duration-300 flex flex-col items-center justify-center text-center space-y-1.5 overflow-hidden shadow-md"
          ...
          :class="[
            isPresetActive(preset)
              ? 'bg-gradient-to-b from-marxi-accent/30 to-marxi-950 border-marxi-accent shadow-glow-red scale-[1.03]'
              : 'bg-marxi-850/90 border-marxi-800 hover:border-marxi-700 hover:bg-marxi-800'
        ...
        >
          ...
          <span class="text-xs font-bold text-white truncate max-w-full">{{ preset.label }}</span>
          <span class="text-[10px] text-gray-400 font-medium truncate max-w-full">{{ preset.subtext }}</span>
    ...
    <div class="bg-marxi-850/95 border border-marxi-800 rounded-2xl p-4 sm:p-5 space-y-4 shadow-xl backdrop-blur-md">
      
      <!-- Top Line: Media Type & Language / Region Switcher -->
      ...
      <div class="flex flex-wrap items-center justify-between gap-2.5 pt-2 border-t border-marxi-800/60 text-xs">
        ...
        <!-- Media Type Selector (All / Movies / TV) -->
        <div class="flex items-center space-x-1.5 bg-marxi-900 p-1.5 rounded-xl border border-marxi-800 shrink-0">
        ...
        <div class="flex items-center space-x-1 bg-marxi-900 p-1 rounded-xl border border-marxi-800 shrink-0">
          <button 
            ...
            class="px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all min-h-[36px]"
            class="px-3 py-1 rounded-lg text-xs font-bold transition-all min-h-[32px]"
            :class="[
              selectedType === t.id
                ? 'bg-marxi-accent text-white shadow-glow-red font-black'
        ...
        <!-- Regional Languages Pills -->
        <div class="flex items-center space-x-1.5 overflow-x-auto hide-scrollbar py-0.5">
          <button 
            v-for="lang in LANGUAGES" 
            :key="lang.id"
            @click="setLanguage(lang.id)"
            class="px-3 py-1.5 rounded-xl text-xs font-semibold shrink-0 transition-all border min-h-[36px] flex items-center space-x-1.5"
            :class="[
              selectedLanguage === lang.id
                ? 'bg-marxi-gold text-black border-marxi-gold font-bold shadow-md scale-[1.02]'
      ...
      <!-- Middle Line: Genre Badges Carousel -->
      <div class="space-y-2 pt-2 border-t border-marxi-800/60">
        <div class="flex items-center justify-between">
          <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Genres</span>
          <span v-if="selectedGenreId" class="text-[10px] text-marxi-gold font-semibold">
            Filtered by Genre
          </span>
        </div>
        ...
        <div class="flex items-center space-x-1.5 overflow-x-auto hide-scrollbar pb-1">
          <button 
            @click="setGenre(null)"
            class="px-3 py-1.5 rounded-xl text-xs font-medium shrink-0 transition-all border min-h-[34px]"
            :class="[
              selectedGenreId === null
                ? 'bg-white text-black font-bold border-white shadow-md'
                : 'bg-marxi-800 text-gray-300 border-marxi-700 hover:text-white'
            ]"
          >
            All Genres
          </button>
          ...
          <button 
            ...
            :class="[
              selectedGenreId === genre.id
                ? 'bg-marxi-accent text-white font-bold border-marxi-accent shadow-glow-red'
                : 'bg-marxi-800 text-gray-300 border-marxi-700 hover:text-white'
      ...
      <!-- Bottom Line: Sort Dropdown & 18+ Mature Toggle -->
      <div class="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-marxi-800/60 text-xs">
        <div class="flex items-center space-x-2">
          ...
          <select 
            ...
            class="bg-marxi-800 border border-marxi-700 text-white text-xs font-semibold rounded-xl px-3 py-1.5 focus:outline-none focus:border-marxi-accent min-h-[36px]"
        ...
        <label class="flex items-center space-x-2 cursor-pointer bg-marxi-800 hover:bg-marxi-700 px-3 py-1.5 rounded-xl border border-marxi-700 transition-colors shrink-0 min-h-[36px]">
          <input 
            type="checkbox" 
            v-model="includeAdult" 
            @change="fetchMedia(true)"
            class="w-4 h-4 accent-marxi-accent rounded cursor-pointer"
          />
          <span class="font-bold text-red-400 flex items-center space-x-1">
            <span>🔞</span>
            <span>Include 18+ / Mature Content</span>
          </span>
        </label>
      </div>
    ...
    <!-- Active Filters Feedback Summary -->
    <div class="flex items-center justify-between text-xs text-gray-400 border-b border-marxi-800 pb-3">
      <span>
        Showing <strong class="text-white">{{ items.length }}</strong> results 
        <template v-if="activeCategoryTitle"> for <strong class="text-marxi-gold">{{ activeCategoryTitle }}</strong></template>
      </span>
      <span v-if="totalPages > 1" class="text-gray-500">Page {{ page }} of {{ totalPages }}</span>
        <!-- Genres Strip -->
        <div class="space-y-1.5">
          <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Genres</span>
          <div class="flex items-center space-x-1.5 overflow-x-auto hide-scrollbar pb-1">
    ...
    <div v-else-if="items.length > 0" class="space-y-6 sm:space-y-8">
      ...
      <div v-if="hasMore" class="flex justify-center pt-2 sm:pt-4">
        <button
          ...
          class="px-8 py-3.5 bg-marxi-accent hover:bg-marxi-accentHover text-white font-bold text-sm rounded-xl shadow-glow-red flex items-center space-x-2 transition-all disabled:opacity-50 min-h-[44px]"
    ...
    >
      <template #action>
        <button 
          @click="resetFilters" 
          class="px-6 py-2.5 bg-marxi-accent text-white font-bold text-xs rounded-xl shadow-glow-red min-h-[44px]"
...
const PRESETS = [
  { id: 'k-drama', label: 'K-Dramas', subtext: 'Korean Hits', icon: '🇰🇷', lang: 'ko', type: 'tv' as MediaType },
  { id: 'anime', label: 'Anime', subtext: 'Japanese Anime', icon: '🇯🇵', lang: 'ja', genre: 16, type: 'tv' as MediaType },
  { id: 'bollywood', label: 'Bollywood', subtext: 'Hindi Cinema', icon: '🇮🇳', lang: 'hi', type: 'movie' as MediaType },
  { id: 'action', label: 'Action', subtext: 'Blockbusters', icon: '💥', genre: 28, type: 'movie' as MediaType },
  { id: 'romance', label: 'Romance', subtext: 'Love & Drama', icon: '💖', genre: 10749, type: 'movie' as MediaType },
  { id: 'scifi', label: 'Sci-Fi', subtext: 'Space & Future', icon: '🚀', genre: 878, type: 'movie' as MediaType },
  { id: 'turkish', label: 'Turkish', subtext: 'Dramas & Series', icon: '🇹🇷', lang: 'tr', type: 'tv' as MediaType },
  { id: '18plus', label: '18+ Mature', subtext: 'Adult Content', icon: '🔞', adult: true, type: 'movie' as MediaType }
...
const activeCategoryTitle = computed(() => {
  ...
  if (selectedLanguage.value) {
    const langObj = LANGUAGES.find(l => l.id === selectedLanguage.value);
    if (langObj) parts.push(langObj.label);
  }
  if (selectedGenreId.value) {
    const genreObj = allGenres.value.find(g => g.id === selectedGenreId.value);
    if (genreObj) parts.push(genreObj.name);
  }
  if (includeAdult.value) parts.push('18+ Mature');
  return parts.join(' • ');
});
