...
import { _ as _sfc_main$2 } from './ContentCard-tSabKuUV.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6 sm:space-y-8" }, _attrs))}><div class="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 border-b border-marxi-800 pb-5"><div><h1 class="text-2xl sm:text-4xl font-display font-black text-white tracking-tight"> Explore Movies </h1><p class="text-xs sm:text-sm text-gray-400 mt-1"> Browse popular, trending, top-rated, and upcoming movies streaming on RHFlix. </p></div><div class="flex items-center space-x-1.5 sm:space-x-2 bg-marxi-850 p-1.5 rounded-2xl border border-marxi-800 overflow-x-auto hide-scrollbar"><!--[-->`);
      ...
      if (unref(loading) && unref(items).length === 0) {
        ...
        if (unref(hasMore)) {
          ...
          _push(`<span>${ssrInterpolate(unref(loadingMore) ? "Loading..." : "Load More Movies")}</span></button></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
...
//# sourceMappingURL=movies-BsqY1oT2.mjs.map
