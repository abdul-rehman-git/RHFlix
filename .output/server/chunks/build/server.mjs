...
const _routes = [
  ...
  },
  {
    name: "index",
    path: "/",
    component: () => import('./index-CXoQtUsC.mjs')
  },
  {
    name: "movies",
    path: "/movies",
...
const useTmdb = () => {
  ...
  const discoverMedia = async (options = {}) => {
    ...
    try {
      const res = await fetchFromTmdb(`/discover/${type}`, params);
  ...
  const getVideos = async (type, id) => {
    ...
    try {
      const res = await fetchFromTmdb(`/${type}/${id}/videos`);
      return res.results || [];
    } catch (_) {
      return [];
    }
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/categories",
        class: ["text-sm font-medium transition-colors hover:text-white flex items-center space-x-1", [unref(route).path.startsWith("/categories") ? "text-white font-semibold" : "text-gray-400"]]
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<span${_scopeId}>Categories</span><span class="px-1.5 py-0.2 text-[9px] bg-marxi-gold text-black font-extrabold rounded uppercase"${_scopeId}>Hub</span>`);
          } else {
            return [
              createVNode("span", null, "Categories"),
              createVNode("span", { class: "px-1.5 py-0.2 text-[9px] bg-marxi-gold text-black font-extrabold rounded uppercase" }, "Hub")
            ];
          }
        }),
        _: 1
      ...
      if (unref(mobileMenuOpen)) {
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/categories",
          onClick: ($event) => mobileMenuOpen.value = false,
          class: ["flex items-center justify-between px-4 py-3 rounded-xl text-sm font-semibold transition-all", [unref(route).path.startsWith("/categories") ? "bg-marxi-accent text-white font-bold shadow-glow-red" : "text-gray-300 hover:bg-marxi-850 hover:text-white"]]
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="flex items-center space-x-3"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-marxi-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"${_scopeId}></path></svg><span${_scopeId}>Categories &amp; Regions</span></div><span class="px-2 py-0.5 text-[9px] bg-marxi-gold text-black font-extrabold rounded uppercase"${_scopeId}>Hub</span>`);
            } else {
              return [
                createVNode("div", { class: "flex items-center space-x-3" }, [
                  (openBlock(), createBlock("svg", {
                    xmlns: "http://www.w3.org/2000/svg",
                    class: "h-5 w-5 text-marxi-gold",
                    fill: "none",
                    viewBox: "0 0 24 24",
                    stroke: "currentColor"
                  }, [
                  ...
                  createVNode("span", null, "Categories & Regions")
                ]),
                createVNode("span", { class: "px-2 py-0.5 text-[9px] bg-marxi-gold text-black font-extrabold rounded uppercase" }, "Hub")
              ];
            }
          }),
          _: 1
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  const _component_NuxtLink = __nuxt_component_0$1;
  _push(`<footer${ssrRenderAttrs(mergeProps({ class: "bg-marxi-950 border-t border-marxi-800/80 pt-12 pb-8 text-gray-400 text-sm" }, _attrs))}><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8"><div class="md:col-span-2 space-y-4"><div class="flex items-center space-x-2.5"><img${ssrRenderAttr("src", _imports_0)} alt="RHFlix Logo" class="h-7 sm:h-8 w-auto object-contain"><span class="font-display font-extrabold text-xl text-white"> RHFlix<span class="text-marxi-accent">.</span></span></div><p class="text-gray-400 max-w-sm text-xs leading-relaxed"> Watch movies, TV series, and trending blockbusters online in 4K HDR. Enjoy fast, bufferless cinema entertainment anytime on RHFlix. </p><div class="flex items-center space-x-3 text-xs text-gray-500"><span>Ultra HD Playback</span> • <span>Fast Streaming</span> • <span>Multi-Device</span></div></div><div><h4 class="text-white font-semibold text-xs tracking-wider uppercase mb-3">Quick Navigation</h4><ul class="space-y-2 text-xs"><li>`);
  _push(ssrRenderComponent(_component_NuxtLink, {
    to: "/",
