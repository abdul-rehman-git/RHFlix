...
import { defineComponent, computed, ref, watch, unref, withCtx, createTextVNode, toDisplayString, openBlock, createBlock, createVNode, isRef, mergeProps, useSSRContext } from 'vue';
...
import { _ as _sfc_main$5 } from './ContentRow-DhalaQaz.mjs';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const isEpisodeUnreleased = (episode) => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.episodes, (episode) => {
        ...
        if (!isEpisodeUnreleased(episode)) {
          ...
          _push(`<div class="w-full sm:w-auto px-4 py-2.5 bg-marxi-900/80 text-amber-400 font-bold text-xs rounded-xl flex items-center justify-center space-x-1.5 border border-amber-500/30 shrink-0 min-h-[44px] cursor-not-allowed"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 fill-current" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path></svg><span>Coming Soon</span></div>`);
        }
        _push(`</div>`);
      });
      _push(`<!--]--></div>`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const show = ref(null);
    const isUnreleased = computed(() => isComingSoon(show.value));
    const topCast = ref([]);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        if (unref(isUnreleased)) {
          ...
          _push(`<span class="px-2.5 py-1 bg-marxi-accent text-white font-bold rounded-lg uppercase tracking-wider text-[10px]"> TV Series </span>`);
        }
        if (unref(show).vote_average && unref(show).vote_average > 0) {
        ...
        if (unref(show).first_air_date && unref(isUnreleased)) {
          _push(`<span class="px-2.5 py-1 bg-marxi-800 rounded-lg text-gray-300 border border-marxi-700"> Expected: ${ssrInterpolate(unref(show).first_air_date)}</span>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(show).number_of_seasons && !unref(isUnreleased)) {
        ...
        if (unref(show).number_of_episodes && !unref(isUnreleased)) {
        ...
        if (unref(isUnreleased)) {
          _push(`<div class="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-amber-500/15 via-amber-900/10 to-transparent border border-amber-500/30 text-amber-200 text-xs sm:text-sm flex items-start space-x-3 shadow-lg"><div class="w-8 h-8 rounded-xl bg-amber-500/20 flex items-center justify-center shrink-0 text-amber-400 mt-0.5"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div><div class="space-y-1 text-left"><p class="font-bold text-white text-xs sm:text-sm"> This series has not aired yet. </p><p class="text-gray-300 text-xs leading-relaxed"> Scheduled premiere: <strong class="text-amber-400">${ssrInterpolate(_ctx.formattedReleaseDate(unref(show)))}</strong>. Episode streaming will unlock once the season premieres. You can watch the official trailer or bookmark it to your list! </p></div></div>`);
        } else {
          _push(`<!---->`);
        }
...
//# sourceMappingURL=_id_-WAhazsws.mjs.map
