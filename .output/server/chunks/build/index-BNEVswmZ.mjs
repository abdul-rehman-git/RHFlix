...
import { b as _sfc_main$1$1 } from './ContentCard-C9DSSU5M.mjs';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_ComingSoonModal = _sfc_main$1$1;
      if (__props.item) {
        ...
        if (unref(releaseYear)) {
          _push(`<span class="text-gray-300 font-medium">${ssrInterpolate(unref(isUnreleased) ? `Expected ${unref(releaseYear)}` : unref(releaseYear))}</span>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (!unref(isUnreleased)) {
          ...
          _push(`<button class="px-5 py-3 sm:px-6 sm:py-3.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-extrabold rounded-xl flex items-center space-x-2 shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 min-h-[44px]"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"></path></svg><span>Coming Soon \u2022 Details</span></button>`);
        }
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: unref(detailsUrl),
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(watchHistoryItems) && unref(watchHistoryItems).length > 0) {
        ...
        ssrRenderList(unref(watchHistoryItems), (item) => {
          _push(`<div class="group relative flex-none w-44 sm:w-56 bg-marxi-850 rounded-2xl overflow-hidden border border-marxi-800 hover:border-marxi-accent transition-all duration-300 shadow-md snap-start"><button class="absolute top-2 right-2 z-20 w-7 h-7 rounded-full bg-black/60 hover:bg-red-600 text-white flex items-center justify-center transition-colors shadow-md opacity-0 group-hover:opacity-100" title="Remove from Continue Watching"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: getWatchUrl(item),
...
//# sourceMappingURL=index-BNEVswmZ.mjs.map
