...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const { getItemProgress } = useWatchHistory();
    const mode = ref("auto");
    ...
    const resumeTimeOffset = ref(null);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(showResumePrompt) && unref(savedProgressSeconds) > 10) {
        _push(`<div class="absolute top-3 left-3 right-3 sm:left-6 sm:right-auto z-40 max-w-md bg-marxi-950/95 border border-marxi-gold/40 rounded-xl p-3 shadow-2xl flex items-center justify-between gap-3 text-xs text-white backdrop-blur-md"><div class="flex items-center space-x-2 min-w-0"><span class="w-2 h-2 rounded-full bg-marxi-gold animate-ping shrink-0"></span><div class="truncate"><span class="text-marxi-gold font-bold block">Resume Playback?</span><span class="text-[11px] text-gray-300">Last watched at ${ssrInterpolate(unref(formattedResumeTime))}</span></div></div><div class="flex items-center space-x-2 shrink-0"><button class="px-3 py-1.5 bg-marxi-gold hover:bg-amber-400 text-black font-bold rounded-lg text-xs transition-colors shadow-md min-h-[32px]"> Resume </button><button class="px-2 py-1.5 text-gray-400 hover:text-white text-xs"> Dismiss </button></div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(currentState) === "LOADING" || unref(currentState) === "FALLBACK") {
...
//# sourceMappingURL=PlaybackPlayer-Q3GyOdp2.mjs.map
