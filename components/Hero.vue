    ...
    <div class="relative max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8 flex items-center">
      <div class="max-w-2xl space-y-3 sm:space-y-4 pt-12 sm:pt-0">
        ...
        <div class="flex items-center space-x-2.5 sm:space-x-3 text-xs font-semibold">
          <span v-if="isUnreleased" class="px-2.5 py-1 bg-gradient-to-r from-amber-500 to-amber-600 text-black rounded-md tracking-wider uppercase font-black text-[10px] shadow-md flex items-center space-x-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 fill-current" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
            </svg>
            <span>Coming Soon</span>
          </span>
          <span v-else class="px-2.5 py-1 bg-marxi-accent text-white rounded-md tracking-wider uppercase font-bold text-[10px]">
            Featured {{ item.title ? 'Movie' : 'TV Show' }}
          </span>
          
          <div v-if="item.vote_average && item.vote_average > 0" class="flex items-center space-x-1 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-md text-marxi-gold border border-white/10">
          ...
          <span v-if="releaseYear" class="text-gray-300 font-medium">
            {{ isUnreleased ? `Expected ${releaseYear}` : releaseYear }}
          </span>
        </div>
        ...
        <div class="pt-2 sm:pt-3 flex flex-wrap items-center gap-3 sm:gap-4">
          ...
          <!-- Unreleased: Coming Soon Info / Trailer -->
          <button 
            v-else
            @click="isComingSoonModalOpen = true"
            class="px-5 py-3 sm:px-6 sm:py-3.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-extrabold rounded-xl flex items-center space-x-2 shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 min-h-[44px]"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
            </svg>
