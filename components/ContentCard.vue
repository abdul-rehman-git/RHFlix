<template>
  <div 
    class="group relative bg-marxi-850 rounded-2xl overflow-hidden shadow-md hover:shadow-glow-card border border-marxi-800/60 hover:border-marxi-700 transition-all duration-300 transform hover:-translate-y-1.5"
  ...
  >
    ...
    <NuxtLink :to="detailUrl" class="block relative aspect-[2/3] w-full overflow-hidden bg-marxi-800">
      ...
      <div class="absolute inset-0 bg-gradient-to-t from-marxi-950 via-marxi-950/20 to-transparent opacity-60 group-hover:opacity-80 transition-opacity duration-300"></div>
      ...
      <!-- Quick Bookmark Button (Touch Friendly 44x44px) -->
      <button 
        @click.prevent="toggleMyList(item)"
